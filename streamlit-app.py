...
ALL_NODES_FILE = INSTALL_DIR / "allnodes.txt" # 存储所有节点的纯文本文件
STREAMLIT_APP_FILE = INSTALL_DIR / "app.py" # Streamlit应用文件
...
def parse_args():
    ...
    parser.add_argument("action", nargs="?", default="install",
                        choices=["install", "status", "update", "del", "uninstall", "cat", "web"],
                        help="操作类型: install(安装), status(状态), update(更新), del(卸载), cat(查看节点), web(启动Web界面)")
...
def print_info():
    ...
    print("\033[36m│ \033[32m版本: 25.7.1 (支持Streamlit Web界面)                 \033[36m│\033[0m")
    print("\033[36m╰───────────────────────────────────────────────────────────────╯\033[0m")
...
def print_usage():
    ...
    print("  \033[36mpython3 script.py web\033[0m                 - 启动Web界面查看节点信息")
...
def generate_links(domain, port_vm_ws, uuid_str):
    ...
    # 保存所有链接到文件
    all_links_text = "\n".join(all_links) + "\n"
    (INSTALL_DIR / "allnodes.txt").write_text(all_links_text)
    (INSTALL_DIR / "jh.txt").write_text(all_links_text) 
    ...
    list_content_color_file.append("\033[36m│ \033[32mWeb界面: \033[0mpython3 " + os.path.basename(__file__) + " web")
    ...
    # 生成Streamlit应用文件
    generate_streamlit_app(domain, port_vm_ws, uuid_str, all_links, link_names)
    ...
    print(f"\033[36m│ \033[32mWeb界面应用已保存到: \033[0m{STREAMLIT_APP_FILE}")
    ...
    print("\033[36m│ \033[32m使用 \033[33mpython3 " + os.path.basename(__file__) + " web\033[32m 启动Web界面查看节点\033[0m")
...
# 生成Streamlit应用文件
def generate_streamlit_app(domain, port_vm_ws, uuid_str, all_links, link_names):
...
# 页面配置
st.set_page_config(
    page_title="ArgoSB 节点信息",
    page_icon="✨",
    layout="wide",
)

# 标题
st.title("✨ ArgoSB 节点信息")

# 基本信息
with st.expander("🔍 基本信息", expanded=True):
    col1, col2 = st.columns(2)
    col1.markdown(f"**域名 (Domain):** \\n`{domain}`")
    col2.markdown(f"**UUID:** \\n`{uuid_str}`")
    
    col3, col4 = st.columns(2)
    col3.markdown(f"**本地Vmess端口 (Local VMess Port):** \\n`{port_vm_ws}`")
    col4.markdown(f"**WebSocket路径 (WS Path):** \\n`{ws_path}`")

# 节点列表
with st.expander("📡 所有节点列表", expanded=True):
    for i, (link, name) in enumerate(zip(all_links, link_names), 1):
        st.markdown(f"### {i}. {name}")
        st.text_area("", link, height=100, key=f"link_{i}")
        ...
        with col1:
            st.markdown(download_button(link, f"node_{i}.vmess", "📥 下载"), unsafe_allow_html=True)
        with col2:
            st.button(f"📋 复制节点 {i}", on_click=lambda l=link: st.session_state.update({"clipboard": l}))
        
        st.markdown("---")

# 纯文本节点
with st.expander("📋 纯文本节点列表（可直接复制）", expanded=False):
    all_links_text = "\\n".join(all_links)
    st.text_area("", all_links_text, height=300)
    
    col1, col2 = st.columns([1, 4])
    with col1:
        st.markdown(download_button(all_links_text, "all_nodes.txt", "📥 下载全部"), unsafe_allow_html=True)
    with col2:
        st.button("📋 复制全部节点", on_click=lambda t=all_links_text: st.session_state.update({"clipboard": t}))

# 状态栏
with st.expander("💡 使用提示", expanded=False):
    st.markdown("""
    1. **节点使用说明：**
       - 点击节点下方的"📋 复制节点"按钮复制单个节点链接
       - 点击"📥 下载"按钮下载节点配置文件
       - 点击"📋 复制全部节点"按钮复制所有节点链接
       - 点击"📥 下载全部"按钮下载所有节点配置文件
    
    2. **连接指南：**
       - 将复制的链接导入到支持VMess协议的客户端中（如Clash、V2Ray等）
       - 导入后选择相应节点即可开始使用
    
    3. **常见问题：**
       - 如果节点无法连接，请检查域名是否正确解析
       - 确保客户端支持WebSocket和TLS协议
       - 如遇问题，请尝试切换不同的节点
    """)

# 页脚
st.markdown("---")
st.caption("✨ ArgoSB Python3 自定义域名版 ✨")
st.caption("© 2025 康康")
...
'''
    ...
    write_debug_log(f"Streamlit应用文件已生成: {STREAMLIT_APP_FILE}")
...
def start_web_interface():
    if not STREAMLIT_APP_FILE.exists():
        print("\033[31mStreamlit应用文件不存在，请先安装服务。\033[0m")
        sys.exit(1)
    
    print("\033[32m正在启动Web界面...\033[0m")
    print("\033[33m请在浏览器中访问: http://localhost:8501\033[0m")
    print("\033[33m按 Ctrl+C 停止Web界面。\033[0m")
    
    try:
        subprocess.run(["streamlit", "run", str(STREAMLIT_APP_FILE)], check=True)
    except subprocess.CalledProcessError as e:
        print(f"\033[31m启动Streamlit失败: {e}\033[0m")
        print("\033[33m请确保已安装Streamlit: pip install streamlit\033[0m")
        sys.exit(1)
    except KeyboardInterrupt:
        print("\n\033[32mWeb界面已停止。\033[0m")
        sys.exit(0)
...
def check_status():
    ...
    if sb_running and cf_running and LIST_FILE.exists():
        ...
        print("\033[36m├───────────────────────────────────────────────────────────────┤\033[0m")
        print("\033[36m│ \033[33m节点信息:\033[0m")
        if (INSTALL_DIR / "allnodes.txt").exists():
            with open(INSTALL_DIR / "allnodes.txt", 'r') as f:
                links = f.read().splitlines()
                print(f"\033[36m│ \033[32m共生成了 {len(links)} 个节点\033[0m")
                print(f"\033[36m│ \033[32m使用 \033[33mpython3 {os.path.basename(__file__)} cat\033[32m 查看所有节点\033[0m")
                print(f"\033[36m│ \033[32m使用 \033[33mpython3 {os.path.basename(__file__)} web\033[32m 启动Web界面查看节点\033[0m")
        else:
            print("\033[36m│ \033[31m节点文件未找到，请重新安装或检查配置。\033[0m")
        print("\033[36m╰───────────────────────────────────────────────────────────────╯\033[0m")
        return True
    ...
    if args.action == "install":
        ...
        if INSTALL_DIR.exists() and CONFIG_FILE.exists() and SB_PID_FILE.exists() and ARGO_PID_FILE.exists():
            ...
            if check_status():
                 ...
                 print("\033[32m如需查看节点信息，可执行: python3 " + os.path.basename(__file__) + " web\033[0m")
            else:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1: # 如果只运行脚本名，没有其他参数
        ...
        if INSTALL_DIR.exists() and CONFIG_FILE.exists() and SB_PID_FILE.exists() and ARGO_PID_FILE.exists():
            ...
            print(f"\033[33m如需查看节点Web界面，请运行: python3 {script_name} web\033[0m")
            check_status()
        else:
