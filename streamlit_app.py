...
def main():
    ...
    st.markdown(f"""
    ### 功能说明
    - 此应用可创建临时 SSH 会话（使用 tmate），并将会话信息保存到 `{SSH_INFO_FILE}`。
    - 新增：**在当前运行环境（容器/服务器）直接执行命令**（在页面顶部输入要执行的命令）。
    - 使用 tmate 版本: **{TMATE_VERSION}**
    """)
    
    # 强烈安全警告
    st.warning("""
    **安全提示（务必阅读）**
    - 页面上的“执行命令”将直接在运行 Streamlit 的服务器上执行任意 shell 命令。
    - 仅在您完全信任并控制的开发/测试环境使用此功能。
    - 切勿在生产、共享或对外暴露的环境中启用该功能。
    """)
...
"""
    ...
    if execute_button:
        # 执行前给出确认提示（Streamlit 不支持 modal 二次确认，这里直接执行）
        st.info("开始执行命令...")
        cwd = str(USER_HOME) if use_cwd_home else None
        if show_env:
            st.subheader("环境变量（部分）")
            # 展示常用的部分环境变量，避免暴露全部敏感信息
    ...
    st.title("tmate 管理")
    st.markdown(f"""
    此部分保留了原有 tmate 功能：下载并启动 tmate，会话信息将保存到 `{SSH_INFO_FILE}`。
    """)
