...
export port_vm_ws=${vmpt:-'22335'}  ### 使用固定端口22335 ###
export port_hy2=${hypt:-''}
export port_tu=${tupt:-''}
export port_xh=${xhpt:-''}
export port_an=${anpt:-''}
export ym_vl_re=${reym:-'www.linux.org'}  ### 默认SNI域名 ###
export argo=${argo:-'y'}  ### 启用Argo隧道 ###
export ARGO_DOMAIN=${agn:-'jd.ppwq.us.kg'}  ### 固定Argo域名 ###
export ARGO_AUTH=${agk:-'eyJhIjoiMTcxNjEzYjZkNTdjZTY2YzdhMWQ2OGQzMGEyMDBlYTYiLCJ0IjoiMmE1ZTA0ZDQtZjMwMy00ZjAzLTgwM2ItNjc2NmRkYTc2MTU4IiwicyI6Ik4ySTROV00yTkRjdE5EVTBZaTAwWVdaakxXSTRPVEV0TkdGbE16WmhZVE5qT0dWaSJ9'}  ### Argo认证Token ###
export ipsw=${ip:-''}
showmode(){
...
if [ -n "$vmp" ]; then
vmp=vmpt
echo "Vmess-ws端口：$port_vm_ws"  ### 使用固定端口，不再随机生成 ###
if [ -e "$HOME/agsb/xray" ]; then
cat >> "$HOME/agsb/xr.json" <<EOF
        {
            "tag": "vmess-xr",
...
name='固定'
### 优化Argo隧道启动参数，提高稳定性 ###
nohup "$HOME/agsb/cloudflared" tunnel --no-autoupdate --edge-ip-version auto --protocol http2 --connection-timeout 30s --retries 5 run --token "${ARGO_AUTH}" >/dev/null 2>&1 &
echo "${ARGO_DOMAIN}" > "$HOME/agsb/sbargoym.log"
echo "${ARGO_AUTH}" > "$HOME/agsb/sbargotoken.log"
else
name='临时'
nohup "$HOME/agsb/cloudflared" tunnel --url http://localhost:"${port_vm_ws}" --edge-ip-version auto --no-autoupdate --protocol http2 --connection-timeout 30s --retries 5 > "$HOME/agsb/argo.log" 2>&1 &
fi
...
if [ -n "${ARGO_DOMAIN}" ] && [ -n "${ARGO_AUTH}" ]; then
### 优化Argo隧道重启参数 ###
echo '@reboot /bin/sh -c "nohup $HOME/agsb/cloudflared tunnel --no-autoupdate --edge-ip-version auto --protocol http2 --connection-timeout 30s --retries 5 run --token $(cat $HOME/agsb/sbargotoken.log 2>/dev/null) >/dev/null 2>&1 &"' >> /tmp/crontab.tmp
else
if [ -e "$HOME/agsb/xray" ]; then
echo '@reboot /bin/sh -c "nohup $HOME/agsb/cloudflared tunnel --url http://localhost:$(grep -A2 vmess-xr $HOME/agsb/xr.json | tail -1 | tr -cd 0-9) --edge-ip-version auto --no-autoupdate --protocol http2 --connection-timeout 30s --retries 5 > $HOME/agsb/argo.log 2>&1 &"' >> /tmp/crontab.tmp
else
echo '@reboot /bin/sh -c "nohup $HOME/agsb/cloudflared tunnel --url http://localhost:$(grep -A2 vmess-sb $HOME/agsb/sb.json | tail -1 | tr -cd 0-9) --edge-ip-version auto --no-autoupdate --protocol http2 --connection-timeout 30s --retries 5 > $HOME/agsb/argo.log 2>&1 &"' >> /tmp/crontab.tmp
fi
fi
fi
...
echo "VPS系统：$op"
echo "CPU架构：$cpu"
echo "默认伪装域名（SNI）：$ym_vl_re（可通过reym变量自定义）"
